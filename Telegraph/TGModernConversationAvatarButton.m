...
- (void)setAvatarUrls:(NSArray *)urls
{
    ...
    for (NSString *uri in urls)
    {
        ...
        if (uri.length == 0)
        {
            [avatarView loadGroupPlaceholderWithSize:CGSizeMake(17.0f, 17.0f) conversationId:[_avatarConversationIds[i] longLongValue] title:_avatarTitles[i] placeholder:placeholder];
        }
        else
        {
...
- (void)layoutSubviews
{
    ...
    if (UIInterfaceOrientationIsPortrait(_orientation) || TGIsPad())
    {
        ...
        if (iosMajorVersion() >= 7 && iosMajorVersion() < 11)
            rtlOffset = -27.0f;
        if (TGAppDelegateInstance.rootController.isRTL) {
            rtlOffset = 10.0f;
        }
